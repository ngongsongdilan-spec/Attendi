...
import { Users, FolderKanban, CheckCircle, Clock, Building, Briefcase, UserPlus, FileText } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-[#1E1B4B] to-[#8B5CF6] rounded-2xl p-6 text-white">
        ...
        <p className="text-[#C4C1FB]">{user?.department || 'Computer Engineering'} Department</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-sm border border-[#C8C5D0] p-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button className="flex items-center gap-2 p-3 bg-[#EDEEEF] rounded-xl hover:bg-[#E7E8E9]"><UserPlus size={18} className="text-[#3B82F6]" /> Add Student</button>
              <button className="flex items-center gap-2 p-3 bg-[#EDEEEF] rounded-xl hover:bg-[#E7E8E9]"><Briefcase size={18} className="text-[#8B5CF6]" /> Add Lecturer</button>
              <button className="flex items-center gap-2 p-3 bg-[#EDEEEF] rounded-xl hover:bg-[#E7E8E9]"><FolderKanban size={18} className="text-[#F59E0B]" /> New Project</button>
              <button className="flex items-center gap-2 p-3 bg-[#EDEEEF] rounded-xl hover:bg-[#E7E8E9]"><FileText size={18} className="text-[#10B981]" /> Report</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-[#C8C5D0] p-6 mt-6">
            ...
            {mockProjects.filter(p => p.department === user?.department).map(p => (
              <div key={p.id} className="flex items-center justify-between p-3 bg-[#EDEEEF] rounded-xl mb-2">
                <div><p className="font-medium">{p.title}</p><p className="text-xs text-[#47464F]">Progress: {p.progress}%</p></div>
        ...
        <div>
          <ActivityFeed activities={[{ user: 'System', action: 'Department updated', time: '1 hour ago' }]} />
        </div>
      </div>
    </div>
  );
