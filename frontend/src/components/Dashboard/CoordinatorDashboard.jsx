...
import { mockStudents, mockProjects, mockTasks } from '../../Data/mockData';

const CoordinatorDashboard = ({ user }) => {
  const [stats, setStats] = useState({ students: 0, lecturers: 0, projects: 0, tasks: 0 });
  const departmentName = user?.department || 'Computer Engineering';
  ...
  const statCards = [
    ...
    { icon: Building, label: 'Department', value: departmentName, color: 'primary' },
  ];

  return (
    <div className="space-y-4 md:space-y-6 max-w-7xl mx-auto">
      <div className="bg-gradient-to-r from-[#1E1B4B] to-[#8B5CF6] rounded-xl md:rounded-2xl p-4 md:p-6 text-white">
        <h2 className="text-xl md:text-2xl font-bold">Welcome, {user?.first_name ? `${user.first_name} ${user.last_name || ''}`.trim() : 'Coordinator'}</h2>
